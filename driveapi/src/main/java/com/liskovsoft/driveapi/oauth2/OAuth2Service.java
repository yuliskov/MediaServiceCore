...
import com.liskovsoft.driveapi.app.AppService;
import com.liskovsoft.driveapi.common.helpers.RetrofitHelper;
import com.liskovsoft.driveapi.oauth2.models.auth.AccessToken;
import com.liskovsoft.driveapi.oauth2.models.auth.UserCode;
import com.liskovsoft.driveapi.oauth2.models.info.AccountInt;
import com.liskovsoft.driveapi.oauth2.models.info.AccountsList;
...
public class OAuth2Service {
    ...
    private final OAuth2Api mAuthApi;
    ...
    private OAuth2Service() {
        mAuthApi = RetrofitHelper.withJsonPath(OAuth2Api.class);
    ...
    public UserCode getUserCode() {
        Call<UserCode> wrapper = mAuthApi.getUserCode(OAuth2ApiHelper.CLIENT_ID, OAuth2ApiHelper.DRIVE_SCOPE);
    ...
    private AccessToken getAccessToken(String deviceCode) {
        Call<AccessToken> wrapper = mAuthApi.getAccessToken(OAuth2ApiHelper.CLIENT_ID, OAuth2ApiHelper.CLIENT_SECRET, deviceCode, OAuth2ApiHelper.GRANT_TYPE);
        return RetrofitHelper.get(wrapper);
    }

    public AccessToken getAccessTokenRaw(String rawJsonAuthData) {
        Call<AccessToken> wrapper = mAuthApi.getAccessTokenRaw(rawJsonAuthData);
        return RetrofitHelper.get(wrapper);
    }
    ...
    public AccessToken updateAccessToken(String refreshToken) {
        Call<AccessToken> wrapper = mAuthApi.updateAccessToken(OAuth2ApiHelper.CLIENT_ID, OAuth2ApiHelper.CLIENT_SECRET, OAuth2ApiHelper.GRANT_TYPE, refreshToken);
        return RetrofitHelper.get(wrapper);
    }
    ...
    public List<AccountInt> getAccounts() {
        Call<AccountsList> wrapper = mAuthApi.getAccountsList(OAuth2ApiHelper.getAccountsListQuery());
